...
if CLIENT then
  ...
  local function DrawHeartbeat(x, y, w, h, health, percent, armour, kevlar, hide, text)
    local offset = 0;
	...
	if text then
	  local s = "FINE"
	  if percent <= HOLOHUD.ELEMENTS:GetElementUserConfigParam("health", "health_thres_crit") then
	    s = "DANGER"
	  elseif percent <= HOLOHUD.ELEMENTS:GetElementUserConfigParam("health", "health_thres_warn") then
	    s = "CAUTION"
	  end
      HOLOHUD:DrawText(x + 10, y + 4, s, "holohud_weapon_name", HEALTH:GetHealthColour(), nil, nil, nil);
	end
  ...
  function HOLOHUD.ELEMENTS.HEALTH:HeartratePanel(health, percent, armour, kevlar, hide, text)
    ...
    HOLOHUD:DrawFragment(HEALTH_PANEL_OFFSET, ScrH() - (HEART_PANEL_H + HEALTH_PANEL_OFFSET), HEART_PANEL_W + width + offset, HEART_PANEL_H, DrawHeartbeat, PANEL_NAME, health, percent, armour, kevlar, hide, text);
